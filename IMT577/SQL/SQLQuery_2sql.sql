...
SELECT P.PRODUCTID AS DIMPRODUCTID, P.PRODUCTID, PT.PRODUCTTYPEID, PC.PRODUCTCATEGORYID,
    P.Product,
    PT.ProductType,
    PC.ProductCategory,
    P.Price,  
   ...
   FROM STAGE_PRODUCT_CATEGORY PC
    LEFT JOIN STAGE_PRODUCT_TYPE PT ON PC.PRODUCTCATEGORYID = PT.PRODUCTCATEGORYID
    LEFT JOIN STAGE_PRODUCT P ON P.PRODUCTTYPEID = PT.PRODUCTTYPEID
...
DimLocationID INT NOT NULL Foreign Key REFERENCES Dim_Location(DimLocationID), 
...
SELECT 
    L.DIMLOCATIONID,
    S.STOREID,
    concat('Store Number',S.STORENUMBER),
    S.STORENUMBER,
    S.STOREMANAGER

 FROM STAGE_STORE S 
    JOIN DIM_LOCATION L ON L.ADDRESS = S.ADDRESS
    AND L.CITY = S.CITY
...
FROM STAGE_CUSTOMER C
    JOIN DIM_LOCATION L ON C.ADDRESS = L.ADDRESS
...
C.CHANNELID, 
CT.CHANNELCATEGORYID, 
C.CHANNEL, 
CT.CHANNELCATEGORY
 
 FROM STAGE_CHANNEL C 
    JOIN STAGE_CHANNEL_CATEGORY CT ON CT.CHANNELCATEGORYID = C.CHANNELCATEGORYID 
);
